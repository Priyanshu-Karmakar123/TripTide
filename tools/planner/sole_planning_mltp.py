...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="mistral")  # default switched to mistral
        ...
        for number, query_data in enumerate(tqdm(query_data_list, desc="Processing data")):
            ...
            while True:
                if args.strategy in ['react', 'reflexion']:
                    planner_results, scratchpad = planner.run(reference_information, query_data['annotation_plan'], query_data['disruption_info'])
                else:
                    planner_results = planner.run(
                        query_data['annotation_plan'],
                        query_data['disruption_info'],
                        query_data['reference_information'],
            ...
            # Use per-model filename instead of gpt4o
            result_file = os.path.join(output_dir, f'{args.model_name}_orig_generated_plan_{number+1}.json')
